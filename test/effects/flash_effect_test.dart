...
void main() {
  ...
  group('FlashEffect', () {
    testWithGame<FlameGame>('is not active initially', FlameGame.new, (
      game,
    ) async {
    ...
    testWithGame<FlameGame>('becomes active on BossFlashEvent', FlameGame.new, (
      game,
    ) async {
      final flash = FlashEffect();
      await game.ensureAdd(flash);
    ...
    testWithGame<FlameGame>('deactivates after flash duration', FlameGame.new, (
      game,
    ) async {
      final flash = FlashEffect();
      await game.ensureAdd(flash);
