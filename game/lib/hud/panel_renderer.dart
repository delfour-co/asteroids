...
abstract class PanelRenderer {
  ...
  static void drawPanel(ui.Canvas canvas, ui.Rect rect,
      {String title = 'SYSTEM'}) {
    final rrect = ui.RRect.fromRectAndRadius(
        rect, ui.Radius.circular(GameConfig.dialogRadius));
    ...
    canvas.drawRRect(
        ...
        ui.Paint()
          ...
          ..maskFilter = const ui.MaskFilter.blur(
              ui.BlurStyle.normal, GameConfig.cardGlowBlur));

    // Body fill — surface color
    canvas.drawRRect(
        rrect, ui.Paint()..color = GameConfig.surfaceColor);
    ...
    for (double x = rect.left + 30; x < rect.right - 30; x += 20) {
      canvas.drawLine(
          ui.Offset(x, rect.top), ui.Offset(x, rect.top + 4), tickPaint);
    }
    for (double x = rect.left + 30; x < rect.right - 30; x += 20) {
      canvas.drawLine(
          ui.Offset(x, rect.bottom), ui.Offset(x, rect.bottom - 4), tickPaint);
    }
  ...
  static void drawScanlines(ui.Canvas canvas, ui.Rect rect) {
    final paint = ui.Paint()
      ..color = const ui.Color.fromARGB(5, 0, 255, 255);
    for (double y = rect.top; y < rect.bottom; y += 3) {
      canvas.drawLine(
          ui.Offset(rect.left, y), ui.Offset(rect.right, y), paint);
    }
  }
  ...
  static void drawGlowButton(
      ui.Canvas canvas, ui.Rect rect, String label, ui.Color textColor,
      {double fontSize = 15, double letterSpacing = 1.5}) {
    final rrect = ui.RRect.fromRectAndRadius(
        rect, ui.Radius.circular(GameConfig.buttonRadius));
    ...
    canvas.drawRRect(
        ...
        ui.Paint()
          ..color = ui.Color.fromARGB(
              (GameConfig.buttonGlowAlpha * 255).toInt(), 0, 255, 255)
          ..style = ui.PaintingStyle.stroke
          ..strokeWidth = 4.0
          ..maskFilter = const ui.MaskFilter.blur(
              ui.BlurStyle.normal, GameConfig.buttonGlowBlur));

    // Fill — cyan 10%
    canvas.drawRRect(
        rrect, ui.Paint()..color = const ui.Color(0x1A00FFFF));
    ...
    final tp = TextPainter(
      text: TextSpan(
        ...
        style: TextStyle(
          ...
          shadows: [
            Shadow(
                color: textColor.withValues(alpha: 0.6), blurRadius: 8),
            Shadow(
                color: textColor.withValues(alpha: 0.3), blurRadius: 16),
          ],
        ),
      ),
      textDirection: TextDirection.ltr,
  ...
  static void drawOutlineButton(
      ui.Canvas canvas, ui.Rect rect, String label, ui.Color textColor,
      {double fontSize = 13, double letterSpacing = 1.5}) {
    final rrect = ui.RRect.fromRectAndRadius(
        rect, ui.Radius.circular(GameConfig.buttonRadius));
  ...
  static void drawTronTitle(ui.Canvas canvas, String text, double cx,
      double cy, double fontSize,
      {double letterSpacing = 4.0}) {
    // Layer 1: Wide glow
    _drawForegroundText(canvas, text, cx, cy, fontSize, letterSpacing,
        ui.Paint()
          ..color = const ui.Color.fromARGB(50, 0, 255, 255)
          ..style = ui.PaintingStyle.stroke
          ..strokeWidth = 4.0
          ..maskFilter =
              const ui.MaskFilter.blur(ui.BlurStyle.normal, 16));
    // Layer 2: Medium glow
    _drawForegroundText(canvas, text, cx, cy, fontSize, letterSpacing,
        ui.Paint()
  ...
  static void _drawForegroundText(ui.Canvas canvas, String text, double cx,
      double cy, double fontSize, double letterSpacing, ui.Paint paint) {
    final tp = TextPainter(
      text: TextSpan(
        text: text,
        style: TextStyle(
  ...
  static void drawTextCentered(ui.Canvas canvas, String text, double x,
      double y, double fontSize, ui.Color color,
      {FontWeight weight = FontWeight.normal,
      bool glow = false,
      double letterSpacing = 0,
      List<Shadow>? shadows}) {
    final tp = layoutText(text, fontSize, color, weight,
        glow: glow, letterSpacing: letterSpacing, shadows: shadows);
    tp.paint(canvas, ui.Offset(x - tp.width / 2, y - tp.height / 2));
  }

  static void drawTextLeft(ui.Canvas canvas, String text, double x, double y,
      double fontSize, ui.Color color,
      {FontWeight weight = FontWeight.normal,
      bool glow = false,
      double letterSpacing = 0}) {
    final tp = layoutText(text, fontSize, color, weight,
        glow: glow, letterSpacing: letterSpacing);
    tp.paint(canvas, ui.Offset(x, y - tp.height / 2));
  }

  static TextPainter layoutText(
      String text, double fontSize, ui.Color color, FontWeight weight,
      {bool glow = false,
      double letterSpacing = 0,
      List<Shadow>? shadows}) {
  ...
  static void drawSeparator(ui.Canvas canvas, double left, double right,
      double y) {
    canvas.drawLine(
      ui.Offset(left, y),
      ui.Offset(right, y),
