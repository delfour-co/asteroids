  ...
  group('Ship rotation via EventBus', () {
    ...
    testWithGame<FlameGame>(
      ...
      (game) async {
        ...
        // Rotate to pi/4
        eventBus.emit(
          JoystickDirectionEvent(angle: pi / 4, isActive: true, intensity: 1.0),
        );
    ...
    testWithGame<FlameGame>('ship unsubscribes on remove', FlameGame.new, (
      game,
    ) async {
      final ship = Ship()..position = Vector2(100, 100);
      await game.ensureAdd(ship);
      ...
      eventBus.emit(
        JoystickDirectionEvent(angle: pi, isActive: true, intensity: 1.0),
      );
